"""
Microservice Database - Persistência de Resultados do Strategy Pipeline

Este módulo gerencia:
- Gravação das últimas 45 previsões por sentido (Horário / Anti-Horário)
- Verificação de acertos (qual dos 3 filtros acertou)
    ...
    def atualizar_resultado(self, previsao_id: int, numero_real: int) -> Dict:
        ...
        with self._lock:
            with sqlite3.connect(self.db_file) as conn:
                ...
                # ===== DEBUG LOG DETALHADO =====
                print(f"[AUDIT] Previsão ID={previsao_id}")
                print(f"[AUDIT]   Posição Partida: {posicao_partida}")
                print(f"[AUDIT]   Força: {forca_vicio} | Centro: {centro_vicio}")
                print(f"[AUDIT]   Região ({len(regiao_vicio)} nums): {regiao_vicio}")
                print(f"[AUDIT]   Número Real: {numero_real}")
                print(f"[AUDIT]   {numero_real} in região? {numero_real in regiao_vicio}")
...
if __name__ == "__main__":
    print("=" * 70)
    print("🗄️  MICROSERVICE DATABASE - Teste (Simplificado - Apenas Rotina)")
    print("=" * 70)
    
    db = MicroservicoDB()
    
    # Criar uma previsão de exemplo (apenas Rotina com 17 números)
    previsao = PrevisaoMicroservico(
        ...
        forca_vicio=12.0,
        centro_vicio=28,
        regiao_vicio=json.dumps([25, 17, 34, 6, 27, 13, 36, 11, 28, 30, 8, 23, 10, 5, 24, 16, 33]),  # 17 números
    ...
    print(f"\n✅ Previsão gravada com ID: {previsao_id}")
    
    # Simular resultado (número caiu na região de 17 números)
