...
| **T5** | O fail-closed cobre **corrupção**, e não só ausência? | **Não.** JSON truncado, `{}` e JSON de tipo errado ⇒ `load()` **não levanta** e devolve estado default. Um 4º cenário (arquivo `chmod 000`) ficou **inconclusivo**, porque o container roda como `uid=0` e a permissão não se aplica | **3 falhas reais + 1 inconclusivo — achado A19** |
