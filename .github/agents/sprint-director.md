...
- **Destravar PRs (o merge é automático, o conflito não):** auto-merge cuida do caminho feliz; seu trabalho é o resto — `gh pr list --json number,baseRefName,mergeable,mergeStateStatus`: **todo PR `SPR-*` com `baseRefName != main` é entrega FALSA** (auto-merge em `spr/*` executa na hora sem CI; exija o PR corretivo → main antes de aceitar o closeout); CONFLICTING → rebase/resolve (ou delega a um executor); vermelho real → fix; issue `main-red` aberta → prioridade máxima (revert ou fix-forward).
- **Última milha no closeout:** merge em `extension/` só vira entrega quando o checkout do operador (`Desktop\Roleta Cloud`) recebeu `git pull` + Reload no Chrome (até o SPR-D4 automatizar); host é automático via shim (D2). Registre pendências de última milha na linha do board.
